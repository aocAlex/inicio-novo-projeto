...
export const MainApp = () => {
  ...
  console.log('📱 MainApp - Initial Render State:', {
    user: !!user,
    authLoading,
  ...
  if (authLoading) {
    console.log('⏳ MainApp - Auth loading state. authLoading:', authLoading);
    return (
  ...
  // Loading state - Super admin status still loading
  if (superAdminLoading) {
    console.log('⏳ MainApp - Super admin status loading state. superAdminLoading:', superAdminLoading);
     return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <Loader2 className="mx-auto h-8 w-8 animate-spin text-blue-600" />
  ...
  console.log('➡️ MainApp - Checking for super admin redirect. isSuperAdmin:', isSuperAdmin, 'currentPath:', location.pathname);
  console.log('➡️ MainApp - Checking for super admin redirect. isSuperAdmin:', isSuperAdmin, 'currentPath:', location.pathname);
  console.log('➡️ MainApp - Checking for super admin redirect. isSuperAdmin:', isSuperAdmin, 'currentPath:', location.pathname);
  console.log('➡️ MainApp - Checking for super admin redirect. isSuperAdmin:', isSuperAdmin, 'currentPath:', location.pathname);
  // Redirect super admin to super admin users page after loading
  if (isSuperAdmin && location.pathname !== '/superadmin/users') {
    console.log('➡️ MainApp - Redirecting super admin to /superadmin/users');
    return <Navigate to="/superadmin/users" replace />;
  }

  // No user - should not happen in protected route but safety check
  if (!user) {
    console.log('❌ MainApp - No user found in protected route. user:', user);
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
  ...
  // Workspace loading
  if (workspaceLoading) {
    console.log('⏳ MainApp - Workspace loading state. workspaceLoading:', workspaceLoading);
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <Loader2 className="mx-auto h-8 w-8 animate-spin text-blue-600" />
  ...
  if (!currentWorkspace) {
    console.log('⚠️ MainApp - No current workspace available. currentWorkspace:', currentWorkspace);
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <Card className="w-full max-w-md">
          <CardContent className="p-6 text-center">
